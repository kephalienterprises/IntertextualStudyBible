...
\usepackage[raggedright]{titlesec, xcolor}
...
\usepackage{comment} %allows use of \begin{comment} or \comment and \end{comment} or \endcomment for block comments
\usepackage{pdfpages} %simplifies the inclusion of external multi-page PDF documents in LATEX documents (used for cover page)
\usepackage{amssymb} %defines all the symbols found in the AMS symbol fonts msam and msbm (used for copyright symbols, etc.)
\usepackage{dblfnote} %double column footnotes
%\usepackage{fnpos} %footnote positioning commands
\usepackage{fancyhdr}
...
\setlength{\headheight}{25pt} %to avoid all the fancyhdr warnings
%\shorthandoff{"} %trying to avoid babel and bibleref-spanish crashing into each other

\renewcommand*{\marginfont}{\color{gray}\tiny}

%get marginpar to show up on the correct side of page
\makeatletter
\patchcmd{\@addmarginpar}{\ifodd\c@page}{\ifodd\c@page\@tempcnta\m@ne}{}{}
\makeatother
\reversemarginpar

%set marginpar font size to footnote size and prevent page overflow
\let\oldmarginpar\marginpar
\renewcommand{\marginpar}[1]{\oldmarginpar[\raggedleft\tiny\color{gray}{#1}]{\raggedright\tiny\color{gray}{#1}}}


\DFNalwaysdouble %make footnotes (almost) always two column, even if there is enough space to fit them in a single column
...
\vfill
	...
	\vbox{%
		\vspace{\drop}
		{\noindent\HUGE\bfseries\textcolor{red}{Apocalipsis}}\\\\
		{\Large\itshape Leyendo a la Luz \\del Antiguo Testamento}\\[4\baselineskip]		
		{\noindent Arreglado by Caleb George}\par
...
\setbooktitle{Psalms}{Salmo}
\setindexbooktitle{Psalms}{Salmos}
\setbooktitle{SongofSongs}{Cantares}

%\setlength{\emergencystretch}{3em} %this was particularly helpful with 2 column format, kept text from spilling into the space between the columns
...
\makeindex[title=Índice de Escrituras,name=scr,options=-s index_style.ist,intoc]
